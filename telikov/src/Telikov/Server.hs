...
import Telikov.Effects (io2jsm, http2JSM, runM, Exists(..))
...
mainClient = do
  ...
  let sink (Step f) = modifyStore f
      sink (Yield (Exists msg)) = do
        ...
        (s, _) <- Home.eval msg & rpc2JSM & io2jsm & http2JSM & runState curr & runM
        modifyStore (const s)
...
main = do
  ...
  void $ forkIO $ runApp [start (Proxy :: Proxy TelikovBackend)] $ pure ()
